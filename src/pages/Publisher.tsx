...
export function Publisher() {
  ...
  if (view === 'list') {
    return <CampaignList campaigns={campaigns} onNew={startWizard} />
  }
...
/* ─── Campaign List ─────────────────────────────────────────── */
function CampaignList({ campaigns, onNew }: { campaigns: Campaign[]; onNew: () => void }) {
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%', background: '#f9fafb', overflow: 'hidden' }}>
      <div style={{ background: '#fff', borderBottom: '1px solid #e5e7eb', padding: '16px 24px', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
        ...
        <button
          onClick={onNew}
          style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6, padding: '8px 16px', borderRadius: 8, border: 'none', background: '#111827', fontSize: 13, fontWeight: 600, color: '#fff', cursor: 'pointer', fontFamily: 'inherit' }}
        >
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
        <div style={{ display: 'grid', gap: 12 }}>
          {campaigns.map(c => (
            <div key={c.id} style={{ background: '#fff', borderRadius: 10, border: '1px solid #e5e7eb', padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 16 }}>
              <div style={{ width: 36, height: 36, borderRadius: 8, background: c.status === 'active' ? '#f0fdf4' : '#f9fafb', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                {c.status === 'active' ? <Play size={16} style={{ color: '#22c55e' }} /> : <Pause size={16} style={{ color: '#9ca3af' }} />}
              </div>
